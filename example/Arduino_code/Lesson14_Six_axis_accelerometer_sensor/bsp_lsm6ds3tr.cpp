...
static esp_err_t lsm6ds3_getchipID(void)
{
    ...
    uint8_t buf = 0;
    // 使用设备地址直接调用 i2c_read_reg
    err = i2c_read_reg(LSM6DS3_I2C_ADDRESS, WHO_AM_I_ADDRESS, &buf, 1); /*I2C Read read-only register*/
    if (err != ESP_OK)
        return err;
...
esp_err_t lsm6ds3_reset(void)
{
    ...
    uint8_t buf[1] = {0};
    buf[0] = 0x80;
    // i2c_write_reg 现在需要设备地址和寄存器地址，以及数据值（不是指针）
    err = i2c_write_reg(LSM6DS3_I2C_ADDRESS, CTRL3_C_ADDRESS, buf[0]); /*Reboot memory content*/
    if (err != ESP_OK)
        return err;
    vTaskDelay(15 / portTICK_PERIOD_MS);
    err = i2c_read_reg(LSM6DS3_I2C_ADDRESS, CTRL3_C_ADDRESS, buf, 1); /*I2C Read register status*/
    if (err != ESP_OK)
        return err;
    buf[0] |= 0x01;
    err = i2c_write_reg(LSM6DS3_I2C_ADDRESS, CTRL3_C_ADDRESS, buf[0]); /*Software reset*/
    if (err != ESP_OK)
        return err;
    while (buf[0] & 0x01) /*(0: normal mode; 1: reset device)*/
    {
        i2c_read_reg(LSM6DS3_I2C_ADDRESS, CTRL3_C_ADDRESS, buf, 1); /*I2C Read register status*/
        vTaskDelay(15 / portTICK_PERIOD_MS);
    }
    return err;
}
...
esp_err_t lsm6ds3_set_BDU(bool state)
{
    ...
    {
        buf[0] |= 0x40;
        err = i2c_write_reg(LSM6DS3_I2C_ADDRESS, CTRL3_C_ADDRESS, buf[0]); /*1: output registers not updated until MSB and LSB have been read*/
        if (err != ESP_OK)
            return err;
    }
    else
    {
        buf[0] &= 0xBF;
        err = i2c_write_reg(LSM6DS3_I2C_ADDRESS, CTRL3_C_ADDRESS, buf[0]); /*0: continuous update*/
        if (err != ESP_OK)
            return err;
    }
...
esp_err_t lsm6ds3_set_acc_rate(uint8_t rate)
{
    esp_err_t err = ESP_OK;
    uint8_t buf[1] = {0};
    err = i2c_read_reg(LSM6DS3_I2C_ADDRESS, CTRL1_XL_ADDRESS, buf, 1); /*I2C Read register status*/
    if (err != ESP_OK)
        return err;
    buf[0] |= rate;
    err = i2c_write_reg(LSM6DS3_I2C_ADDRESS, CTRL1_XL_ADDRESS, buf[0]); /*Accelerometer Output data rate*/
    if (err != ESP_OK)
        return err;
    return err;
}
...
esp_err_t lsm6ds3_set_acc_fullscale(uint8_t value)
{
    esp_err_t err = ESP_OK;
    uint8_t buf[1] = {0};
    err = i2c_read_reg(LSM6DS3_I2C_ADDRESS, CTRL1_XL_ADDRESS, buf, 1);
    if (err != ESP_OK)
        return err;
    buf[0] |= value;
    err = i2c_write_reg(LSM6DS3_I2C_ADDRESS, CTRL1_XL_ADDRESS, buf[0]); /*Accelerometer full-scale selection*/
    if (err != ESP_OK)
        return err;
    return err;
}
...
esp_err_t lsm6ds3_set_gyr_rate(uint8_t rate)
{
    esp_err_t err = ESP_OK;
    uint8_t buf[1] = {0};
    err = i2c_read_reg(LSM6DS3_I2C_ADDRESS, CTRL2_G_ADDRESS, buf, 1);
    if (err != ESP_OK)
        return err;
    buf[0] |= rate;
    err = i2c_write_reg(LSM6DS3_I2C_ADDRESS, CTRL2_G_ADDRESS, buf[0]); /*Gyroscope Output data rate*/
    if (err != ESP_OK)
        return err;
    return err;
}
...
esp_err_t lsm6ds3_set_gyr_fullscale(uint8_t value)
{
    ...
    if (err != ESP_OK)
        return err;
    buf[0] |= value;
    err = i2c_write_reg(LSM6DS3_I2C_ADDRESS, CTRL2_G_ADDRESS, buf[0]); /*Gyroscope full-scale selection*/
    if (err != ESP_OK)
        return err;
    return err;
}
...
esp_err_t lsm6ds3_set_acc_bandwidth(uint8_t BW0XL, uint8_t ODR)
{
    ...
    if (err != ESP_OK)
        return err;
    buf[0] |= BW0XL;
    err = i2c_write_reg(LSM6DS3_I2C_ADDRESS, CTRL1_XL_ADDRESS, buf[0]); /*Anti-aliasing filter bandwidth selection*/
    if (err != ESP_OK)
        return err;

    err = i2c_read_reg(LSM6DS3_I2C_ADDRESS, CTRL8_XL_ADDRESS, buf, 1);
    if (err != ESP_OK)
        return err;
    buf[0] |= ODR;
    err = i2c_write_reg(LSM6DS3_I2C_ADDRESS, CTRL8_XL_ADDRESS, buf[0]); /*Accelerometer ODR register setting*/
    if (err != ESP_OK)
        return err;
    return err;
}
...
esp_err_t lsm6ds3_set_gry_register4(uint8_t cmd)
{
    ...
    err = i2c_read_reg(LSM6DS3_I2C_ADDRESS, CTRL4_C_ADDRESS, buf, 1);
    if (err != ESP_OK)
        return err;
    buf[0] |= cmd;
    err = i2c_write_reg(LSM6DS3_I2C_ADDRESS, CTRL4_C_ADDRESS, buf[0]); /*Enable gyroscope digital LPF1 (0: disabled; 1: enabled)*/
    if (err != ESP_OK)
        return err;
    return err;
}
...
esp_err_t lsm6ds3_set_gry_register6(uint8_t cmd)
{
    esp_err_t err = ESP_OK;
    uint8_t buf[1] = {0};
    err = i2c_read_reg(LSM6DS3_I2C_ADDRESS, CTRL6_C_ADDRESS, buf, 1);
    if (err != ESP_OK)
        return err;
    buf[0] |= cmd;
    err = i2c_write_reg(LSM6DS3_I2C_ADDRESS, CTRL6_C_ADDRESS, buf[0]); /*this bit must be set to ‘0’ for the correct operation of the device.*/
    if (err != ESP_OK)
        return err;
    return err;
}
...
esp_err_t lsm6ds3_set_gry_register7(uint8_t cmd)
{
    esp_err_t err = ESP_OK;
    uint8_t buf[1] = {0};
    err = i2c_read_reg(LSM6DS3_I2C_ADDRESS, CTRL7_G_ADDRESS, buf, 1);
    if (err != ESP_OK)
        return err;
    buf[0] |= cmd;
    err = i2c_write_reg(LSM6DS3_I2C_ADDRESS, CTRL7_G_ADDRESS, buf[0]); /*Gyroscope digital high-pass filter enable and Gyroscope digital HP filter cutoff selection 260MHZ*/
    if (err != ESP_OK)
        return err;
    return err;
}
...
{
    uint8_t buf[1] = {0};
    i2c_read_reg(LSM6DS3_I2C_ADDRESS, STATUS_REG_ADDRESS, buf, 1); /*I2C Read register status*/
    return buf[0];
}
...
static esp_err_t lsm6ds3_data_read(uint8_t reg_addr, uint8_t *rev_data, uint8_t length)
{
    ...
    {
        err = i2c_read_reg(LSM6DS3_I2C_ADDRESS, reg_addr++, rev_data++, 1); /*I2C Read register data*/
        if (err != ESP_OK)
            return err;
        length--;
