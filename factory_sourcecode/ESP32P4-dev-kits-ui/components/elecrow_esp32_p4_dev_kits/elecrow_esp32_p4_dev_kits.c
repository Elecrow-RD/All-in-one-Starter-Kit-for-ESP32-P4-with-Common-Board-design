...
#include "bsp/elecrow_esp32_p4_dev_kits.h"
#include "bsp/display.h"
#include "bsp/touch.h"
#include "esp_lcd_touch_gt911.h"
#include "bsp_err_check.h"
#include "esp_codec_dev_defaults.h"
#include "bsp/my_codec.h"

static const char *TAG = "ELECROW_P4";
...
esp_err_t bsp_display_new_with_handles(const bsp_display_config_t *config, bsp_lcd_handles_t *ret_handles)
{
    ...
    const gpio_config_t display_gpio_cofig = {
        .pin_bit_mask = ((1ULL << BSP_LCD_UPDN) | (1ULL << BSP_LCD_SHLR)),
        .mode = GPIO_MODE_OUTPUT,
        .pull_up_en = false,
        .pull_down_en = false,
        .intr_type = GPIO_INTR_DISABLE,
    };
    gpio_config(&display_gpio_cofig);
    ...
    gpio_set_level(BSP_LCD_UPDN, 0);
    gpio_set_level(BSP_LCD_SHLR, 1);
...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    if (err != ESP_OK)
    {
        ...
        err = esp_lcd_new_panel_io_i2c((i2c_master_bus_handle_t)i2c_handle, &tp_io_config, &tp_io_handle);
        if (err != ESP_OK)
            return err;
